# main.py (Final, Complete, JobQueue-Free Async Timer, MarkdownV2 Fix, SyntaxWarning Fix)
...
async def timeout_fast_round_task(chat_id: int, context: ContextTypes.DEFAULT_TYPE, time_limit: int):
    ...
    try:
        ...
        try:
            if 'last_message_id' in state:
                await context.bot.edit_message_caption(
                    ...
                    caption=rf"⏱️ **TIME UP\!** The answer was **{state['answer'].upper()}**\.", # Used rf
                    reply_markup=None,
        ...
        await context.bot.send_message(
            chat_id=chat_id, 
            text=rf"⏱️ **Round {state['current_round']} Timeout\!** The answer was **{state['answer'].upper()}**\.", # Used rf
            parse_mode='MarkdownV2' 
        )
...
async def start_fast_round(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading_message = await context.bot.send_message(
        chat_id, 
        rf"🚀 **Fast Round {state['current_round']}/{state['max_rounds']}**: Downloading and analyzing image\.\.\.", # Used rf
        parse_mode='MarkdownV2' 
    )
...
async def end_fast_game_logic(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    end_message = (
        ...
        rf"**Total Correct**: **{rounds_played} / {state['max_rounds']}**\n"
    ...
    await context.bot.send_message(chat_id, end_message, parse_mode='MarkdownV2', reply_markup=keyboard) 
...
async def fastgame_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(rf"🚀 **FAST MODE** started\! Get ready for 10 rounds, 10 seconds each\!", parse_mode='MarkdownV2') # Used rf
    await start_fast_round(chat_id, context)
...
async def end_fast_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    if chat_id not in fast_games:
        await update.message.reply_text(r"No active fast game to end\.") # Used r
        return
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        r"Get started with `/game`\!" # Used r
    )
...
async def howtoplay_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rules_text = (
        ...
        r"• **10 seconds** time limit per image\.\n" 
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    about_text = (
        rf"**🤖 About {BOT_USERNAME}**\n\n"
        r"**Version:** v3\.1 \(Async Timer Fix\)\n" # Used r
...
async def skip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game_state:
        ...
        await update.message.reply_text(
            rf"🛑 **G A M E   S K I P P E D** 🛑\n The correct solution was: **{correct_answer}** \({letter_count} letters\)\.", # Used rf
            parse_mode='MarkdownV2', 
            reply_markup=keyboard 
        )
...
async def simple_hint_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hint_message = (
        rf"💡 **HINT USED\!** \(\-{points_cost} Points\)\n" # Used rf
...
async def game_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in active_games:
        await update.message.reply_text(r"A normal game session is currently active\. Please submit your guess or use `/skip`\.", parse_mode='MarkdownV2') # Used r
        return
    
    if chat_id in fast_games:
        await update.message.reply_text(r"A fast game session is currently active\. Please finish it or use `/endfastgame`\.", parse_mode='MarkdownV2') # Used r
        return
    ...
    await update.message.reply_text(
        r"**📸 I N I T I A T E   G A M E**\n\nSelect a precision level for the image analysis \(Difficulty\):", # Used r
    ...
    if data == 'game_end':
        ...
        if game_state:
            ...
            try:
                ...
                await context.bot.send_message(chat_id, rf"The game has ended\. The correct answer was: **{correct_answer}** \({letter_count} letters\)\.", parse_mode='MarkdownV2', reply_markup=keyboard) # Used rf
...
async def check_guess_and_update_score(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if chat_id in fast_games:
        ...
        if user_guess == correct_answer or correct_answer in user_guess.split() or user_guess in correct_answer.split():
            ...
            try:
                if 'last_message_id' in state:
                    await context.bot.edit_message_caption(
                        chat_id=chat_id,
                        message_id=state['last_message_id'],
                        caption=rf"✅ **GUESSED\!** The answer was **{correct_answer.upper()}**\.", # Used rf
                        reply_markup=None,
                        parse_mode='MarkdownV2' 
                    )
            except Exception:
            ...
            await context.bot.send_message(
                chat_id, 
                rf"✅ **BINGO\!** **\+{reward}** Points\. Correct Answer: **{correct_answer.upper()}**\.", # Used rf
                parse_mode='MarkdownV2' 
            )
    ...
    if user_guess == correct_answer or correct_answer in user_guess.split() or user_guess in correct_answer.split():
        ...
        try:
            ...
            await context.bot.send_message(chat_id, rf"{caption}\n\(Original image file unavailable\)\.", parse_mode='MarkdownV2', reply_markup=keyboard) # Used rf
...
async def my_score_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(rf"💰 Your current score is: **{score:,} Points**\.", parse_mode='MarkdownV2') # Used rf
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    leaderboard_text = r"**🏆 Global Leaderboard \(Top 10\)**\n\n" # Used r
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if last_daily_claim and (now_utc - last_daily_claim).total_seconds() < 24 * 3600:
        ...
        await update.message.reply_text(
            rf"⏳ You have already claimed your daily bonus\.\n" # Used rf
            rf"Come back in **{hours}h {minutes}m** to claim your next **\+{DAILY_BONUS_POINTS}** points\!", # Used rf
            parse_mode='MarkdownV2' 
        )
    else:
...
async def album_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if filter_letter:
        ...
        album_title = rf"🖼️ Your Pixel Album \(Categories starting with **{filter_letter}**\)" # Used rf
        ...
        album_title = r"🖼️ Your Pixel Album \(First 20 Items\)" # Used r
    else:
        # Default view for small album is all items
        collected_categories = all_collected_categories
        album_title = r"🖼️ Your Pixel Album \(All Collected Items\)" # Used r
...
async def donate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    donate_text = (
        f"**💖 Support {BOT_USERNAME} Development 💖**\n\n" # Left as f-string as it has no custom escapes
