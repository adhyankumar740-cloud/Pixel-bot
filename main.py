...
async def timeout_fast_round_task(chat_id: int, context: ContextTypes.DEFAULT_TYPE, time_limit: int):
    ...
    try:
        ...
        try:
            if 'last_message_id' in state:
                await context.bot.edit_message_caption(
                    ...
                    caption=f"⏱️ **TIME UP\!** The answer was **{state['answer'].upper()}**\.", # Escaped ! and .
                    reply_markup=None,
        ...
        await context.bot.send_message(
            chat_id=chat_id, 
            text=f"⏱️ **Round {state['current_round']} Timeout\!** The answer was **{state['answer'].upper()}**\.", # Escaped ! and .
            parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
        )
...
async def start_fast_round(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading_message = await context.bot.send_message(
        chat_id, 
        f"🚀 **Fast Round {state['current_round']}/{state['max_rounds']}**: Downloading and analyzing image\.\.\.", # FIX 1: Changed to MarkdownV2 and escaped ...
        parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
    )
    ...
    if not pixelated_img_io:
        await context.bot.send_message(chat_id, f"❌ **Error**: Image acquisition failed\. Fast Game aborted\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
    ...
    try:
        ...
        await context.bot.send_message(chat_id, "❌ **Error**: Image transmission failed\. Fast Game aborted\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
        del fast_games[chat_id]
...
async def fastgame_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🚀 **FAST MODE** started\! Get ready for 10 rounds, 10 seconds each\!", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
    await start_fast_round(chat_id, context)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "**Collection & Shop:**\n"
        f"• `/album [Letter]` \- View your image categories\. Use a letter \(e\.g\.\, `/album A`\) to filter your large collection\.\n" # Escaped \- and () and .
...
async def handle_game_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading_message = await context.bot.send_message(
        ...
        f"**⏳ Downloading and analyzing image\. Please wait\.\.\.**", # Escaped .
        parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
    )
    ...
    if not pixelated_img_io:
        ...
        await context.bot.send_message(chat_id, f"❌ **Error**: Image acquisition failed\. Details: {escaped_error_message}", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
        return
    ...
    try:
        ...
        await context.bot.send_message(chat_id, "❌ **Error**: Image transmission failed\. Challenge cancelled\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
...
async def check_guess_and_update_score(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if chat_id in fast_games:
        ...
        if user_guess == correct_answer or correct_answer in user_guess.split() or user_guess in correct_answer.split():
            ...
            try:
                if 'last_message_id' in state:
                    await context.bot.edit_message_caption(
                        chat_id=chat_id,
                        message_id=state['last_message_id'],
                        caption=f"✅ **GUESSED\!** The answer was **{correct_answer.upper()}**\.", # Escaped ! and .
                        reply_markup=None,
                        parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
                    )
            except Exception:
            ...
            await context.bot.send_message(
                chat_id, 
                f"✅ **BINGO\!** **\+{reward}** Points\. Correct Answer: **{correct_answer.upper()}**\.", # Escaped ! and + and .
                parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
            )
...
async def my_score_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"💰 Your current score is: **{score:,} Points**\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2 and escaped .
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if last_daily_claim and (now_utc - last_daily_claim).total_seconds() < 24 * 3600:
        ...
        if success:
            ...
            await update.message.reply_text("❌ Failed to claim daily bonus due to a database error\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
...
async def album_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if filter_letter:
        ...
        album_title = f"🖼️ Your Pixel Album \(Categories starting with **{filter_letter}**\)" # Escaped ()
        ...
        album_title = "🖼️ Your Pixel Album \(First 20 Items\)" # Escaped ()
    else:
        # Default view for small album is all items
        collected_categories = all_collected_categories
        album_title = "🖼️ Your Pixel Album \(All Collected Items\)" # Escaped ()
...
def check_owner_wrapper(handler):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_user.id == BROADCAST_ADMIN_ID:
            ...
            await update.message.reply_text("❌ You are not authorized to use this command\.", parse_mode='MarkdownV2') # FIX 1: Changed to MarkdownV2
    return wrapper
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id in list(known_users): # Iterate over a copy
        try:
            ...
            final_broadcast_message = f"**📣 B R O A D C A S T**\n\n{escaped_broadcast_message}"
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.effective_chat:
        try:
             await context.bot.send_message(
                update.effective_chat.id,
                "⚠️ **An unexpected error occurred\.** The developer has been notified\. Please try again or use `/help`\." # Escaped .
                , parse_mode='MarkdownV2' # FIX 1: Changed to MarkdownV2
            )
        except Exception:
            pass
