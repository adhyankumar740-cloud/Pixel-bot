        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                UPDATE game_scores SET 
                    ...
                    current_streak = CASE 
                        -- Check if solved within 48 hours to maintain streak. A simple check for streak maintenance.
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if last_claim:
        # Ensure last_claim has UTC timezone info
        if last_claim.tzinfo is None:
            last_claim = last_claim.replace(tzinfo=pytz.utc)
        
        time_since_claim = current_time - last_claim
        
        # Check if 23 hours and 59 minutes have passed (effectively 24h cooldown)
        if time_since_claim < timedelta(hours=23, minutes=59):
...
async def howtoplay_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    how_to_play_text = (
        ...
        "**3. Using Utilities**\n"
        "   - **Complimentary Clue**: Use `/hint` once per game to get the image category.\n"
...
async def simple_hint_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Gives the category of the image as a complimentary hint."""
    chat_id = update.effective_chat.id
    game_state = active_games.get(chat_id)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "`/howtoplay` - Learn the rules and mechanics of the game. (NEW)\n\n"
        ...
        "`/photoid` - Get the File ID of a photo/sticker. (NEW)\n"
    )
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    about_text = (
        ...
        "**Version**: *6.0 (Standardisation/Fixes Update)*"
    )
...
def main() -> None:
    ...
    # --- NEW COMMANDS HANDLERS ADDED HERE ---
