# main.py (Final Version: All Features Included, Pixelation Increased, Shop/Album/Start Upgraded)
...
GAME_LEVELS = {
    ...
    'hard': {'downscale_factor': 15, 'points': 70, 'max_hints': 3, 'hint_cost': 10}, # Increased from 12
    'medium': {'downscale_factor': 10, 'points': 50, 'max_hints': 3, 'hint_cost': 10}, # Increased from 7
    'easy': {'downscale_factor': 5, 'points': 30, 'max_hints': 3, 'hint_cost': 10} # Increased from 3
}
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not data:
                ...
                # Default to a time 2 days ago for easy initial daily claim
...
async def fetch_and_pixelate_image(difficulty: str) -> tuple[io.BytesIO | None, str | None, str | None, str | None]:
    ...
    try:
        ...
        # Handle float factors by rounding appropriately for PIL size calculations
        if isinstance(downscale_factor, float):
            downscale_factor = max(1.0, downscale_factor) # Ensure it's not too small
        ...
        # Ensure small size is at least 1x1
...
# ------------------------------------------------------------------
# --- PLAYER PROFILE AND SHOP COMMANDS ---
# ------------------------------------------------------------------
...
async def album_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Shows the user their collected stickers/images using an inline keyboard."""
    ...
    for category_name, emoji in sorted_categories:
        status_emoji = emoji if category_name in collected_categories else "❓"
        button_text = f"{status_emoji} {category_name.title()}"
        
        # Callback will trigger a simple pop-up status
        callback_data = f'album_view_{category_name}' 
        
        current_row.append(InlineKeyboardButton(button_text, callback_data=callback_data))
        
        if len(current_row) == 2: # 2 buttons per row for a compact "school" look
    ...
    album_text += f"**Total Unlocked**: {unlocked_count}/{total_count}\n"
    album_text += "Tap on a category to see its collection status."
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check for callback query first (for inline button presses)
    ...
    if current_score < cost:
        response = f"❌ Purchase failed! You need **{cost} pts**, but only have **{current_score} pts**."
        if update.callback_query:
    ...
    if item_id == '1': # Flashlight (Reveal 2 letters)
        ...
        if game_state and '_' in game_state['hint_string']:
            ...
            letters_to_reveal = 2 # Reveal 2 random masked letters
            ...
            response += f"🔦 **Flashlight Activated!** (Revealed {letters_to_reveal} letters)\n**Progress**: `{game_state['hint_string']}`"
            
            # Check if game is won by hint
            if '_' not in game_state['hint_string']:
                del active_games[chat_id]
                response += "\n\n❌ Word fully revealed by power-up! Game Over (No points awarded)."
        else:
            response += "🔦 **Flashlight**: There is no active game or the word is already complete."
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get last claim time (The complexity of fetching profile data is managed inside the function)
    rank, score, solved_count, current_streak, last_claim = await get_user_profile_data(user_id)
    
    # Ensure last_claim is timezone aware for comparison
...
async def game_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Prompts user to select game difficulty, only showing Extreme if APIs are available."""
    chat_id = update.effective_chat.id
    ...
    # Check if any image API is available to allow Extreme difficulty
    ...
    if is_extreme_available:
        # Add Extreme only if an API is available
...
async def handle_game_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Only edit if the previous message was the selection menu, not the /start welcome message
    if not query.message.text.startswith("👋 **Welcome"):
        ...
        await context.bot.send_message(chat_id, f"Game starting... Difficulty: **{difficulty.upper()}**", parse_mode='Markdown')
    ...
    game_keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"💡 Get Hint (-{level_data['hint_cost']} pts) ({level_data['max_hints'] - active_games[chat_id]['hints_taken']} left)", callback_data='game_hint')],
        [InlineKeyboardButton("🛑 End Game (Reveal Answer)", callback_data='game_end')]
    ])
...
async def handle_core_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handles general inline button presses like 'help_menu'."""
    query = update.callback_query
    await query.answer()
    ...
    if data == 'help_menu':
        # Call the help command to send the message
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    start_message = (
        f"👋 **Welcome, {user_name}!** I'm PixelBot, ready to test your visual skills. ✨\n\n"
        "Your mission: Guess the hidden word in the **pixelated photo**!\n\n"
        "**How to Play:**\n"
        "1. Start a game with `/game` and choose your challenge level.\n"
        "2. Guess the word by simply typing it in the chat.\n"
        "3. Earn points and unlock stickers for your `/album`!\n\n"
        "👉 **Ready?** Tap the button below to start your first game!"
    )
    
    keyboard = [
        [InlineKeyboardButton("📸 Start Guessing Game (Easy)", callback_data='game_easy')],
        [InlineKeyboardButton("📚 View Commands (/help)", callback_data='help_menu')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "`/profile` - Show your personal profile/report card.\n\n"
        
        "**📸 Image Guessing Game**\n"
        "`/game` - Start a new game and select difficulty.\n"
        "`/myscore` - Check your current game score.\n"
        "`/leaderboard` - Show the top 10 players.\n"
        "`/album` - Show collected stickers (Inline).\n"
        "`/skip` - Skip the current image.\n"
        "`/hint` - Get a free category clue.\n\n"
        
        "**🛒 Shop & Economy**\n"
        "`/daily` - Claim free daily points.\n"
        "`/shop` - Open the Pixel Shop.\n"
        "`/buy <id>` - Purchase a power-up.\n"
    )
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    about_text = (
        "❤️ **About Bot** ❤️\n\n"
        "I am a custom core utility bot, featuring a **Pixelated Image Guessing Game** with a **Hint System**, **Shop**, **Daily Rewards**, and **Player Profiles**.\n\n"
        "✨ **Creator**: Adhyan\n"
        "✨ **Version**: 5.1 (Pixelation Increased, Shop/Album/Start Upgraded)\n\n"
    )
...
def main() -> None:
    ...
    # Callback Query Handler for the game and core buttons
    ...
    application.add_handler(CallbackQueryHandler(buy_command, pattern=r'^buy_')) # Re-route buy callback to buy_command
