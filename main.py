# main_core_broadcast_game_dual_api.py 
import os
import logging
import requests
import asyncio
...
UNSPLASH_ACCESS_KEY = os.getenv("UNSPLASH_ACCESS_KEY") # NEW: Unsplash Access Key
...
# ------------------------------------------------------------------
# --- IMAGE GUESSING GAME LOGIC (DUAL API) ---
# ------------------------------------------------------------------
...
async def fetch_image_from_unsplash(query: str):
    ...
    # Fetch a single random photo related to the query
...
async def fetch_and_blur_image(difficulty: str) -> tuple[io.BytesIO | None, str | None, str | None]:
    """Randomly selects an API, fetches, and blurs the image."""
    ...
    # Randomly select one available API fetcher
    fetcher = random.choice(available_apis)

    try:
        # 1. Fetch Image URL and Raw Answer using the chosen API
        ...
        answer = re.sub(r'[^a-zA-Z\s]', '', raw_answer).lower().split()[0].strip()
        if not answer or len(answer) < 3:
             answer = query # Use search query as fallback if answer is too short/empty

        # 3. Download Image Content
        image_data = requests.get(image_url, stream=True, timeout=10).content

        # 4. Process/Blur Image using PIL
        ...
        # Save blurred image to BytesIO buffer
        output = io.BytesIO()
...
# ------------------------------------------------------------------
# --- CORE BOT COMMANDS AND HANDLERS (UNCHANGED) ---
# ------------------------------------------------------------------
...
async def handle_game_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Dual API Fetch and Blur is handled here
