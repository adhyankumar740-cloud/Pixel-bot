# main.py (Final, Complete, Speed-Optimized, with JobQueue, Fast Mode, Segmentation and Expanded Categories)
...
from telegram.ext import (
    ...
    ContextTypes, CallbackQueryHandler, JobQueue  # <-- FIX 1: ADD JobQueue HERE
)
...
# Structure: {chat_id: {'user_id': id, 'score': 0, 'current_round': 1, 'max_rounds': 10, 'timer_job_name': 'job_name', 'total_score': 0, 'answer': '...', 'category': '...', 'last_message_id': id}}
...
def escape_markdown_v2(text: str) -> str:
    ...
    Used for AI-generated text to prevent BadRequest errors due to unclosed entities.
    """
...
async def timeout_fast_round_callback(context: ContextTypes.DEFAULT_TYPE):
    """Callback function for the JobQueue to automatically advance the fast game."""
...
async def start_fast_round(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3. Cancel any previous timer job (Crucial: context.job_queue is guaranteed not None now)
    job_name = f'fast_game_timer_{chat_id}'
    current_jobs = context.job_queue.get_jobs_by_name(job_name)
    for job in current_jobs:
        job.schedule_removal()
    
    # 4. Start new timer job
    time_limit = FAST_MODE_SETTINGS['time_limit']
    context.job_queue.run_once(
        timeout_fast_round_callback, 
        time_limit, 
        data={'chat_id': chat_id},
        name=job_name
    )
    state['timer_job_name'] = job_name # Store job name for later removal/check
...
async def end_fast_game_logic(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Cancel any running timer job (Crucial: context.job_queue is guaranteed not None now)
    job_name = f'fast_game_timer_{chat_id}'
    current_jobs = context.job_queue.get_jobs_by_name(job_name)
    for job in current_jobs:
        job.schedule_removal()
...
async def end_fast_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    if chat_id not in fast_games:
        await update.message.reply_text("No active fast game to end.")
        return
        
    await update.message.reply_text("Manually ending the fast game...")
    await end_fast_game_logic(chat_id, context)
...
async def simple_hint_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hint_message = (
        ...
        # Use the escaped hint sentence here
...
async def game_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in fast_games:
        await update.message.reply_text("A fast game session is currently active. Please finish it or use `/endfastgame`.")
        return
...
async def handle_game_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pixelated_img_io:
        await context.bot.send_message(chat_id, f"❌ **Error**: Image acquisition failed. Details: {answer}", parse_mode='Markdown')
        return
    ...
    if not top_scores:
        ...
        for i, (user_id, score) in enumerate(top_scores, 1):
            try:
                ...
                # NOTE: This only works if the command is sent in the *same* chat as the user. 
                # Better to use a lookup table in a real multi-group scenario.
...
async def donate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    donate_text = (
        ...
        "You can scan the QR code below for easy payment. Thank yourself for your support!"
    )
...
def main() -> None:
    ...
    # --- FIX 2: Initialize Application with JobQueue ---
    # We pass a JobQueue instance to the builder so context.job_queue is available
