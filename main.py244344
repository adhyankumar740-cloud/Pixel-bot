...
# ------------------------------------------------------------------
# --- DATABASE CONNECTION AND DATA MANAGEMENT FUNCTIONS (omitted for brevity) ---
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# --- GEMINI AI INTEGRATION AND UTILITY FUNCTIONS (omitted for brevity) ---
# ------------------------------------------------------------------
...
def initialize_gemini_client():
    if not GEMINI_API_KEY:
        ...
        return None
    try:
        # Assuming the library is already installed and imported
...
# ------------------------------------------------------------------
# --- IMAGE GUESSING GAME LOGIC (API Interaction) (omitted for brevity) ---
# ------------------------------------------------------------------
...
async def start_rapid_round(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        ...
        r"Guess the word \*fast\* to continue the streak\! \(No hints\/skips\)" # FIXED: Escaping * and !
    )
...
async def end_rapid_quiz_logic(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    end_message = (
        ...
        rf"**New Balance**: **{new_balance:,}** Points\." # FIXED: Escaping period
    )
...
async def rapidquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        r"Starting 10 rounds, with decreasing time limits per image\. Get ready\!", # FIXED: Escaping period and comma
        parse_mode='MarkdownV2'
    ) 
    await start_rapid_round(chat_id, context)
...
async def simple_hint_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if the update is a callback query, and if so, use the associated message and user.
    if update.callback_query:
        # Use query.message.reply_text/answer for consistency and correct context
    ...
    if not unrevealed_indices:
        await message_sender.reply_text(r"The word is already fully revealed\!", parse_mode='MarkdownV2')
        return
...
async def handle_game_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'game_hint':
        # Delegate to the command handler logic (using query as update source)
    ...
    # --- FIXED: Full MarkdownV2 Escaping in Game Caption ---
    caption = (
...
async def check_guess_and_update_score(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if user_guess == correct_answer or correct_answer in user_guess.split() or user_guess in correct_answer.split():
        ...
        caption = (
            ...
            r"View the original image below\." # FIXED: Escaping period
        )
...
async def handle_core_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'help_menu':
        # Need to pass the query object as if it were a message update
...
def check_owner_wrapper(handler):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id == BROADCAST_ADMIN_ID:
            ...
            await update.message.reply_text(r"❌ You are not authorized to use this command\.", parse_mode='MarkdownV2')
    return wrapper
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text or text.startswith('/'): 
        # Since process_message handles all non-command text/captions,
        # we check for /photoid here if the MessageHandler is configured to catch all text.
