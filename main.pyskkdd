...
async def start_fast_round(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3. Cancel any previous timer job (identified by chat_id)
...
async def end_fast_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if last_daily_claim and (now_utc - last_daily_claim).total_seconds() < 24 * 3600:
        ...
        success = await update_daily_claim(user_id, DAILY_BONUS_POINTS) # Fixed: Added await
        if success:
            new_score = get_user_score(user_id)
            await update.message.reply_text(
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("endfastgame", end_fast_game)) # Fixed: Command for manual stop
